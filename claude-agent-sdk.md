...
**标题：Claude AI智能体应用开发 skill软件开发**

**描述：**

专注 AI Agent 技能（skill）定制开发，已做出基于 Claude Agent SDK 的智能体开发技能，现成作品可先看演示再下单。在你的程序里启动一个会自主干活的 AI 智能体：读写文件、联网搜索、调你的接口；流式返回方便做打字机和进度展示，多轮会话可续接，用 MCP 协议接自定义工具，支持子智能体分头干活，权限控制和钩子方便上生产。文档含官方文档全文和二十多条常见坑。可按你的业务定制，全部源码交付、支持二次开发。

**服务内容：**

- 1、需求沟通梳理：先聊清楚想要的应用形态（读代码助手 / 查数据客服 / 多 AI 协作）、要接入的工具和数据源，列明功能清单后再动手
- 2、skill交付内容和格式：能自主干活的 AI 智能体应用：流式交互、会话管理、自定义工具（MCP）接入、子智能体编排的可运行工程代码
- 3、项目功能/系统开发：按你的业务逻辑定制工具集成、权限钩子、结构化输出与部署方案，含联调测试
- 4、skill源码交付：SKILL.md 技能说明 + 提示词 / 脚本 / 模板 / 代码样例组成的完整技能工程，兼容 codex / workbuddy 等 agent。全部源码 + 安装部署说明 + 使用演示，交付后可自行修改维护
