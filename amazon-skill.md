...
意思是"分析美国站这个商品"，它会自动拉数据、生成报告并存成 Markdown 文档。其他子技能多数也是"给个 ASIN 或关键词 + 站点"就能跑。
...
这套技能不是一个给普通人双击打开的 App，而是给 AI 编程助手（如 Claude Code）用的"技能包"合集，需要放进对应的 skills 目录才能用。更重要的一点是：其中不少子技能依赖第三方数据服务（卖家精灵、Sorftime、Sif、西柚洞察等）的接口或登录态，没配上这些账号，对应的分析就跑不起来。具体每个子技能能做什么、怎么配数据，以各自目录下的 SKILL.md / README.md 为准。
